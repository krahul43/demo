...
const useDriversCommunicationRegistry = () => {
  ...
  const registerForPushNotificationsAsyncRequest = async () =>
    await registerForPushNotificationsAsync();
  useEffect(() => {
    if (!registerSubs && driver?.user)
      (async () => {
        setRegisterSubs(await registerForPushNotificationsAsyncRequest());
      })();
...
export const DriverCommunication = () => {
  useDriversCommunicationRegistry();
  return <Fragment />;
};
